...
v osnovni R sintaksi bi se v `dplyr` sintaksi

`filter(select(flights, UniqueCarrier, DepDelay), DepDelay > 60)`

bralo kot: izberemo stolpca `UniqueCarrier` in `DepDelay` ter ohranimo samo vrstice, ki imajo v stolpcu `DepDelay` več kot 60 minut. Obstaja pa še druga pot, kjer postopek vodimo v vrsticah.
...
Za sortiranje vrednosti v stolcih (spremenljivkah) uporabimo `arrange`.
...
Če želimo obrniti vrstni red (padajoče), uporabimo `desc`.
...
Pogosto se zgodi, da izračunamo novo spremenljivko na osnovi že obstoječih spremenljivk. Tu nam je v pomoč funkcija `mutate`.
...
fl %>%
 ...
 fl <- fl %>%
  mutate(PropAirTravel = ((AirTime/ActualElapsedTime) * 100) %>% round() %>% paste("%")) 
```
...
Lahko vzorčimo kar delež. Tabela ima `nrow(fl)` vrstic. Ena desetina je torej približno `round(nrow(fl)/10)` vrstic.
