---
title: "Uvod v ponovljivo znanost, dplyr in git"
author: "Roman Luštrik in Črt Ahlin"
date: "10.2.2015"
output: 
  html_document:
    theme: cerulean
...
Ko enkrat uvozimo podatke, je naslednji logični korak, da jih podrobneje spoznamo. Za to navadno uporabljamo tabelarične prikaze povzetkov ali pa s pomočjo slik. Za lažjo manipulacijo so razvili paket `dplyr`, ki nam pomaga pri pripravi podatkov za analiziranje.

Na tej delavnici bomo stopali po stopinjah, kot so jih zagazili v [Introduction to dplyr for Faster Data Manipulation in R](https://rpubs.com/justmarkham/dplyr-tutorial).
 ...
 * `filter` izbira vrstice glede na pogoj
 * `select` izbira stolpce glede na pogoj
 * `arrange` sortira podatke
 * `mutate` računa z obstoječimi spremenljivkami in po potrebi dodaja nove
 * `summarize` povzema spremenljivke
...
# ali (za vajo lahko preveriš, če sta oba način res primerljiva)
...
# (za vajo lahko razmisliš kako bi ta izraz zapisal/a z %in%)
```
...
Za razliko od "osnovnega R-ja", znotraj funkcij ni potrebno pisati imena stolpcev v narekovajih. Podobnega pristopa smo vajeni že iz paketa `ggplot2`.

```{r}
fl[1:6, c("Year", "FlightNum", "Origin", "Diverted")]

select(fl, Year, FlightNum, Origin, Diverted)
# priročno, če stolpce izbiramo dinamično na podlagi spremenljivk
...
select(xy, num_range("spec", 1:3), num_range("envir", c(1, 3, 5)))
```
...
# spremembe naredimo v dejanskih podatkih
fl <- fl %>%
...
Če potrebujemo različne statistike glede na spremenljivke, se poslužimo funkcije `summarize`. Ta funkcionalnost je še posebej koristna v navezi z `group_by`.
```{r}
fl %>%
...
Če želimo prešteti število letov (vrstic) v posamezni skupini (oz. kombinaciji skupin, če v `group_by` uporabimo več spremenljivk) lahko uporabimo funkcijo  `n()`.
...
Bližnica za kombinacijo funkcij `n()` in `group_by` je `count`.
...
Za štetje unikatnih vrednosti v posamezni skupni (oz. kombinaciji skupin, če v `group_by` uporabimo več spremenljivk) uporabimo funkcijo `n_distinct()`.
...
```{r}
set.seed(357) # nastavimo zaradi ponovoljivosti
fl %>%
   sample_n(100)
```
...
Včasih imamo podatke v različnih bazah, npr. MySQL, SQLite ipd. `dplyr` omogoča klicanje (ne pa tudi manipuliranja v bazi sami) podatkov, manipulacija pa je možna naknadno v R-ju. Poglejmo si primer z bazo SQLite.
...
# dostop do baze, kjer kličemo tabelo "flights" eksplicitno preko SQL stavka
